...
from difflogic.dataset_utils import RandomlyIterDataset
from difflogic.thutils_rl import monitor_gradrms
...
class TrainerBase(TrainerEnv):
    ...
    def _train_step(self, feed_dict, meters):
        ...
        msg = 'Train: loss={loss:.4f} '.format(loss=loss)
        ...
        if 'closs' in monitors:
            msg += 'closs:{closs:.4f} '.format(closs = monitors['closs'])
        return msg, ret
